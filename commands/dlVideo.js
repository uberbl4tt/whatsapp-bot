...
async function dlVideo(
  ...
  let processing;
  try {
    logger.info("Downloading video", { userId: message.from });
    processing = (await message.reply("memproses url..."));
    logger.debug("Processing URL", { userId: message.from });
    ...
    logger.debug("Starting download process...", { userId: message.from });
    ...
    logger.debug("Verifying video.", { userId: message.from });
    ...
    if (secondDuration > 5460) {
      logger.warn(`Duration too long! (accepted ${secondDuration})`, { userId: message.from });
      await processing.edit(`durasi terlalu panjang! (maks. 1:30:59)`);
      return;
    }

    logger.debug("Video has been verified.", { userId: message.from });
    await processing.edit(
      "video berhasil diverifikasi! menunggu download selesai...",
    );
    await dlProcess;

    logger.debug("Renaming files.", { userId: message.from });
